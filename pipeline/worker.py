...
def _wait_for_complete(path: str, timeout: float = 5.0) -> bool:
    """Wait until file size stabilizes to avoid processing partially written files."""
    last_size = -1
    elapsed = 0.0
...
def worker_loop(audio_dir: str, lang: str, log_queue: Queue, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        for filename in sorted(os.listdir(audio_dir)):
            ...
            if not _wait_for_complete(wav_path):
                log_queue.put(f"[worker] Arquivo {wav_path} incompleto, aguardando próximo ciclo")
                continue
            ...
            try:
                ...
                if speechify_client:
                    ...
                    if audio_data_base64:
                        ...
                        tts_wav = wav_path.replace(".wav", f"_{lang}.wav")
                        with open(tts_wav, "wb") as f:
                            f.write(b64decode(audio_data_base64))
                        log_queue.put(f"[worker] Áudio Speechify salvo em {tts_wav}")
                        mixed_wav = wav_path.replace(".wav", f"_{lang}_mix.wav")
                        subprocess.run([
                            "ffmpeg", "-y", "-i", wav_path, "-i", tts_wav,
                ...
                # --- Garante duracao de 30s ---
                fixed_wav = wav_path.replace(".wav", f"_{lang}_30s.wav")
                subprocess.run([
                    "ffmpeg", "-y", "-i", mixed_wav, "-t", "30",
                    "-af", "apad=pad_dur=30",
                    fixed_wav
                ], check=True)
                ...
                subprocess.run([
                    "ffmpeg", "-y", "-i", fixed_wav, "-c:a", "aac", "-b:a", "128k",
                    "-f", "mpegts", out_ts
                ], check=True)
                ...
                try:
                    if 'tts_wav' in locals() and tts_wav != wav_path:
                        os.remove(tts_wav)
                    if mixed_wav != wav_path:
                        os.remove(mixed_wav)
                    os.remove(fixed_wav)
                except Exception:
                    pass
                ...
                retry_count[filename] = retry_count.get(filename, 0) + 1
                if retry_count[filename] >= 3:
                    log_queue.put(f"[worker] Desistindo de {wav_path} após {retry_count[filename]} falhas")
                    processed.add(filename)
