...
def worker_loop(audio_dir: str, video_dir: str, lang: str, log_queue: Queue, stop_event: threading.Event):
    """
    Loop contínuo que:
    ...
    while not stop_event.is_set():
        ...
        for filename in sorted(os.listdir(audio_dir)):
            ...
            try:
                ...
                aac_path = temp_wav.replace(".wav", ".aac")
                subprocess.run([
                    "ffmpeg", "-y", "-i", temp_wav, "-c:a", "aac", "-b:a", "128k", aac_path
                ], check=True)
                log_queue.put(f"[worker] Áudio convertido para AAC: {aac_path}")

                # --- Combina com o vídeo correspondente ---
                video_file = os.path.join(video_dir, filename.replace(".wav", ".ts"))
                if not os.path.exists(video_file):
                    log_queue.put(f"[worker] Vídeo {video_file} ainda não disponível, aguardando")
                    continue

                out_ts = os.path.join(hls_dir, filename.replace(".wav", ".ts"))
                subprocess.run([
                    "ffmpeg", "-y", "-i", video_file, "-i", aac_path,
                    "-c:v", "copy", "-c:a", "aac", "-shortest", out_ts
                ], check=True)
                with open(playlist, "a") as pl:
                    pl.write(f"#EXTINF:10.0,\n{os.path.basename(out_ts)}\n")
                seq += 1
