...
for base_dir in ("audio_segments", "hls"):
...
def _clean_dirs(audio_dir: str, video_dir: str | None, hls_dir: str) -> None:
    """Remove conteúdos residuais antes de iniciar o pipeline."""
    dirs = [audio_dir, hls_dir]
    if video_dir:
        dirs.insert(1, video_dir)
    for d in dirs:
        if os.path.exists(d):
...
async def start_pipeline(channel: str, lang: str):
    ...
    _clean_dirs(audio_dir, None, hls_dir)
